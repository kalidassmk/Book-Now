...
Maintains an in-memory rolling buffer of OHLCV candles per (symbol, interval)
so that long-running scoring loops (volume_price_analyzer in --daemon mode,
and similar) can call `get_klines(symbol, interval, limit)` without hitting
the REST `/api/v3/klines` endpoint on every tick.

Key design choices
------------------
1. **Single combined stream connection** — one
   `wss://stream.binance.com:9443/stream` socket per process, multiplexing
   up to 1,024 streams. This is the lightest-weight option Binance offers.
2. **Live SUBSCRIBE / UNSUBSCRIBE** — the watch list is mutable. As the
   Spring backend rotates `FAST_MOVE` symbols in/out, we push subscribe and
   unsubscribe frames over the existing socket without reconnecting.
3. **REST seed once per (symbol, interval)** — when a stream is added we do
   one REST `klines` call to fill history; afterwards every closed candle
   from the WS appends to the buffer. ~99% of REST calls disappear.
4. **Backwards compatible API** — `get_klines()` returns the same DataFrame
   shape (`[timestamp, open, high, low, close, volume]`) that
   `volume_price_analyzer.fetch_klines()` returned, so call sites barely
   change.

Reasonable bounds (per Binance docs):
   - 1024 streams per connection
   - Subscribe rate: 5 incoming messages/sec
   - Connection auto-disconnects after 24h — we reconnect cleanly
"""
...
class KlinesCache:
    ...
    Then either:
      ...
      - `cache.get_klines(symbol, interval, limit)` for a non-blocking read
        from the in-memory buffer.

    Thread/concurrency model: all mutations happen on the asyncio event loop
    that calls `start()`. `get_klines()` is safe to call from the same loop.
    """
    ...
    async def release(self, symbols_to_keep: Set[str]) -> None:
        """
        Unsubscribe symbols not in `symbols_to_keep`. Lets the cache shrink
        as the FAST_MOVE list rotates so we don't hit the 1024-stream cap.
        """
    ...
    def get_klines(self, symbol: str, interval: str, limit: int) -> pd.DataFrame:
        """
        Return up to `limit` most recent candles as a DataFrame matching the
        shape that volume_price_analyzer.fetch_klines() returned.
        Returns an empty DataFrame if the cache hasn't been warmed yet.
        """
        sym = symbol.replace("/", "").upper()
