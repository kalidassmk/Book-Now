...
Subscribes once to the public {@code !miniTicker@arr} stream and keeps a
per-symbol dict updated every second. Reads are O(1) and do not hit Binance.

Designed for synchronous Python daemons (profit_020_trend_analyzer,
success_pattern_recorder, etc.) that previously called
{@code ccxt.fetch_tickers()} in a tight loop. The WebSocket runs in a
daemon thread with its own asyncio event loop; consumers just import
the class, call {@code start()} once, and read.
...
Companion to {@code klines_ws_cache.py}, which covers historical kline
buffers for async daemons.
"""
...
logger = logging.getLogger("TickersCache")
...
class TickersCache:
    """
    Thread-backed singleton-ish cache. Create one instance per process;
    `start()` is idempotent. `stop()` is best-effort and called on exit.
    """
