...
public class SymbolInfo {
  ...
  /**
   * @param filterType filter type to filter for (e.g., "LOT_SIZE").
   * @return symbol filter information for the provided filter type.
   */
  public SymbolFilter getSymbolFilter(String filterType) {
    return filters.stream()
        .filter(symbolFilter -> filterType.equals(symbolFilter.getFilterType()))
        .findFirst()
        .orElse(null);
  }
