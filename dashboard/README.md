...
# 3. Start the dashboard (requires Spring Boot bot + Redis running)
npm start
...
```
book-now-v3/dashboard/
│
├── src/                        ← Backend (Node.js)
│   ├── server.js               ← Entry point: wire Express + Socket.io + Redis
│   │
│   ├── config/
│   │   └── settings.js         ← ALL config in one place (env vars + defaults)
│   │
│   ├── redis/
│   │   └── client.js           ← Singleton Redis connection + helpers
│   │
│   ├── services/
│   │   ├── scanner.js          ← Reads fast signals from Redis, sorts & limits
│   │   ├── autoTrader.js       ← Buy/sell decision engine (configurable)
│   │   └── tradeStore.js       ← In-memory state: positions, log, P&L
│   │
│   ├── routes/
│   │   └── api.js              ← REST API: /api/config, /api/buy, /api/sell
│   │
│   └── sockets/
│       └── broadcaster.js      ← Polling loop + Socket.io event hub
│
├── public/                     ← Frontend (Vanilla JS, no framework)
│   ├── index.html              ← HTML shell only (no inline JS/CSS)
│   │
│   ├── css/
│   │   └── dashboard.css       ← All styles in 16 labelled sections
│   │
│   └── js/
│       ├── utils.js            ← Shared: fmt(), fmtTime(), toast(), setLiveStatus()
...
│       ├── orders.js           ← Order placement: quickBuy/Sell, manual form
│       └── main.js             ← Bootstrap: socket connection + event routing
│
